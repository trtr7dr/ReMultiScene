...
class BookScene {
    ...
    set_scenes(id) {
        ...
        this.scenes = {
            Scene: {
                name: 'Main',
                url: '/assets/3d/gugo/' + this.json[this.sname]['gltf'] + '.gltf',
    ...
    camera_create() {
        if (this.scene_id === 1) {
            this.camera = new THREE.PerspectiveCamera(this.json[this.sname]['perspective'], this.w / this.h, 1, 100);
    ...
    kol_set() {
        $.doTimeout('kol_set', 24, function () {
            let kol = mScene.kol['kol' + mScene.page_num];
            if (typeof kol !== 'undefined') {
                if (parseInt(kol['num']) === mScene.page_num && kol.position.z < -2.3) {
                    kol.position.z += 0.2;
                    return true;
                }
            }
            for (const [key] of Object.entries(mScene.kol)) {
                if (mScene.kol[key].position.z > -4.5 && parseInt(mScene.kol[key]['num']) !== mScene.page_num) {
                    mScene.kol[key].position.z -= 0.2;
                    return true;
                }
            };
    ...
    page_next() {
        $.doTimeout('page_set', 24, function () {
            let flag = false;
            if (mScene.pages['page' + mScene.page_num].position.z < 1.5) {
                mScene.pages['page' + mScene.page_num].position.z += 0.1;
                flag = true;
            } else {
                mScene.page_num++;
                flag = false;
    ...
    point_massive() {
        ...
        for (var i = 0; i < 10000; i++) {
            var x = this.rand_int(-2000, 2000);
            var y = this.rand_int(-2000, 2000);
    ...
    cursor_move(z, y) {
        y = this.h / 19000 - y / 9000;
        z = this.w / 19000 - z / 9000;
...
if (/Android|webOS|iPhone|iPad|iPod|BlackBerry|BB|PlayBook|IEMobile|Windows Phone|Kindle|Silk|Opera Mini/i.test(navigator.userAgent)) {
    ...
    $('#mobs').html('<h1>Это не телефонный разговор.</h1>');
     $('#container').remove();
     mScene = null;
} else {
...
$("#container").mousedown(function (e) {
    if (e.button === 0) {
        mScene.page_change('next');
        return false;
    }
    if (e.button === 2) {
        mScene.page_change('prev');
        return false;
    }
    return true;
});
